...
public class DartAcdcGenerator extends DefaultCodegen implements CodegenConfig {
    ...
    public String getHelp() {
        return "Generates a Dart API client integrated with the Dart-ACDC library.\n\n"
                + "Features:\n"
                + "  Authentication (A) - OAuth 2.1 with automatic token refresh\n"
                + "  Caching (C)        - Two-tier (Memory + Disk) with AES-256 encryption\n"
                ...
                + "Package Metadata:\n"
                + "  pubName                   Package name for pubspec.yaml (derived from info.title)\n"
                + "  pubVersion                Package version (derived from info.version, default: 1.0.0)\n"
                + "  pubDescription            Package description (derived from info.description)\n"
                + "  pubAuthor                 Package author\n"
                + "  pubHomepage               Package homepage URL\n\n"
                + "Feature Toggles:\n"
                + "  enableAuthentication       Enable OAuth 2.1 authentication (default: true)\n"
                + "  enableCaching              Enable two-tier caching (default: true)\n"
                + "  enableLogging              Enable configurable logging (default: true)\n"
                + "  enableOfflineSupport       Enable offline detection (default: true)\n"
                + "  enableCertificatePinning   Enable certificate pinning (default: false)\n\n"
                + "Authentication Defaults:\n"
                + "  defaultTokenRefreshUrl     Token refresh endpoint URL\n"
                + "  useSecureTokenStorage      Use platform secure storage (default: true)\n"
                + "  refreshThresholdMinutes    Minutes before expiry to refresh (default: 5, range: 1-60)\n\n"
                + "Caching Defaults:\n"
                + "  defaultCacheTtlHours       Cache TTL in hours (default: 1, range: 1-720)\n"
                + "  cacheDiskSizeMb            Max disk cache in MB (default: 20, range: 1-1024)\n"
                + "  encryptCache               Enable AES-256 cache encryption (default: true)\n"
                + "  enableUserCacheIsolation   Isolate cache per user (default: true)\n\n"
                + "Logging Defaults:\n"
                + "  defaultLogLevel            Log level: none|error|warning|info|debug|verbose (default: info)\n"
                + "  redactSensitiveData        Redact sensitive data in logs (default: true)\n\n"
                + "Code Style:\n"
                + "  serializationLibrary       json_serializable or freezed (default: json_serializable)\n"
                + "  generateInterfaces         Generate abstract API interfaces (default: true)\n"
                + "  dataSourceSuffix           API class name suffix (default: RemoteDataSource)\n"
                + "  generateBarrelExports      Generate barrel export files (default: true)\n\n"
                + "Example YAML config:\n"
